...
class TemplateNode(AttackGraphNode):
    ...
    def __str__(self):
        ...
        #return f"[type: '{self.type}', nlp: '{self.nlp}']"

        # Complete
        return f"[type: '{self.type}', nlp: '{self.nlp}', ioc: '{self.ioc}']"
...
class TechniqueTemplate:
    ...
    NODE_SIMILAR_ACCEPT_THRESHOLD = 0.7
    ...
    def update_template(self, attack_graph: AttackGraph):
        ...
        for node in attack_graph.attackgraph_nx.nodes:
            ...
            for template_node in self.technique_node_list:
                ...
                # Daniel added
                if attack_graph.attackNode_dict[node].type == 'actor' and template_node.type == 'actor':
                    similarity_score = 1
                #
        ...
        for edge in attack_graph.attackgraph_nx.edges:
            ...
            # Daniel added
            self.technique_action_dict[technique_template_edge] = attack_graph.attackgraph_nx.edges[edge]['action']
    ...
    # Recommended using level=complete
    def pretty_print(self, image_name: str = "template.png", level: str = 'complete'):
        G = pgv.AGraph(directed=True)

        d = {}
        if level == 'type':
            number_ner = {}
            for ner in node_shape.keys():
                number_ner[ner] = 0
            for node in self.technique_node_list:
                G.add_node(f"{str(node)}#{number_ner[str(node)]}", shape=node_shape[node.type])
                d[node] = f"{str(node)}#{number_ner[str(node)]}"
                number_ner[str(node)] += 1
            ...
            for edge in self.technique_edge_dict.keys():
                ...
                G.add_edge(d[source], d[sink], count=str(count))
            ...
            for node in self.technique_node_list:
                ...
                if node.ioc:
                    for ioc in node.ioc:
                        d[node].append(f"{ioc}#{idx}")
                        G.add_node(f"{ioc}#{idx}", shape=node_shape[node.type])
                else:
                    for nlp in node.nlp:
                        d[node].append(f"{nlp}#{idx}")
                        G.add_node(f"{nlp}#{idx}", shape=node_shape[node.type])
                idx += 1
            ...
            for node in self.technique_node_list:
                G.add_node(node, shape=node_shape[node.type])
            ...
            for edge in self.technique_edge_dict.keys():
                ...
                sink = self.technique_node_list[edge[1]]
                # TODO
                # G.add_edge(source, sink, count=str(count), label=self.technique_action_dict[edge])
                G.add_edge(source, sink, count=str(count))
